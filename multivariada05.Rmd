...
im <- left_join(im,
          df_nomes %>% 
          select(nome, nome_regiao_imediata),
          by = "nome") %>% 
  relocate(nome, nome_regiao_imediata)


tab_im <- im %>% filter(ano >= 2010) %>% 
  group_by(nome_regiao_imediata, nome, ano) %>% 
  summarise(
  ...
  group_by(ano, nome_regiao_imediata) %>% filter(ano >=2010, ano < 2020) %>% 
  summarise(
...
lavouras <- left_join(lavouras, df_nomes %>% 
  select(nome, nome_regiao_imediata),
  by = "nome") %>% 
  relocate(ano, nome, nome_regiao_imediata)
...
pnae_resumo <- pnae %>% 
  ...
  # drop_na() %>% 
  group_by(ano, nome_regiao_imediata, etapa_ensino) %>% 
  summarise(
...
pnae_resumo$valor_pnae_total <- pnae_resumo %>% 
  rename(nome_regiao_imediata = nome) %>% 
  ungroup() %>% 
...
base_multivariada<-left_join(base_multivariada,
          pnae_resumo %>% 
            rename(nome_regiao_imediata = nome) %>% 
            ungroup() %>% 
            select(nome_regiao_imediata,valor_pnae_total),
          by = "nome_regiao_imediata")
visdat::vis_miss(base_multivariada)
```
...
sisvan_resumo <- sisvan %>% 
  group_by(nome_regiao_imediata, nome) %>% 
  summarise(
...
estado_nutricional <- left_join(estado_nutricional, 
                                df_nomes %>% select(nome, id_municipio_6,nome_regiao_imediata) %>% 
                       rename(id_municipio  = id_municipio_6),
                     by = c("id_municipio")) %>% 
  select(-regiao,-uf,-codigo_uf,-id_municipio,-municipio,-indice_cri,
         -indice_ado,-valor) %>% relocate(ano,nome,nome_regiao_imediata) %>% 
...
consumo <- left_join(consumo, df_nomes %>% select(nome, id_municipio_6, nome_regiao_imediata) %>% 
...
consumo <- consumo %>% 
  pivot_wider(names_from = c(fase_da_vida,tipo_relatorio),
              values_from = percent) %>% 
  janitor::clean_names()
consumo$nome_regiao_imediata %>% unique()
```
...
consumo_estado_nutricional <- left_join(consumo, estado_nutricional,
          by=c("ano","nome","nome_regiao_imediata"))
...
consumo_estado_nutricional_resumo <- left_join(
  ...
  by = c("nome","nome_regiao_imediata"))
...
base_multivariada_final<-left_join(base_multivariada,
  ...
  select(nome_regiao_imediata, cri04pxi_mb,cri04pxi_a,adolaxi_b,adolaxi_a,adulimc_b,adulimc_a,adulimc_obesg_i,adol_ultra,adul_ultra,cri59_ultra,ido_ultra),
  by = "nome_regiao_imediata")
