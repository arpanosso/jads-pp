...
```{r}
glimpse(df_nomes)
df_nomes$nome_regiao_imediata %>% unique()
df_nomes$nome_regiao_intermediaria %>% unique()
```
...
```{r}
cor_matrix <- cor(df %>% 
                    relocate(ends_with("per")), use = "na.or.complete")
corrplot(cor_matrix, method="ellipse")
```
