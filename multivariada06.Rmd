...
pnae_recurso <- pnae_recurso %>% 
  group_by(nome, ano, esfera_governo) %>% 
  summarise(vl_total_escolas = sum(vl_total_escolas))
```
...
```{r}
df_grupo <- data.frame(nome = base_multivariada$nome, grupo)
